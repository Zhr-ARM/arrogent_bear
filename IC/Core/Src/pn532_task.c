 ...
 */void StartPN532_ICCardTask(void const *argument) {
    HAL_UART_Transmit(&huart4, (uint8_t*)"\r\n========================================\r\n", 42, 1000);
    HAL_UART_Transmit(&huart4, (uint8_t*)"[PN532 Task] 任务启动\r\n", 24, 1000);
    HAL_UART_Transmit(&huart4, (uint8_t*)"========================================\r\n\r\n", 44, 1000);
    ...
    if (!PN532_SearchCard(uid, &uid_length)) {
        HAL_UART_Transmit(&huart4, (uint8_t*)"[PN532 Task] 未发现卡片\r\n", 26, 1000);
        goto task_end;
    }
    ...
    if (PN532_ReadPeakInfoSelective(&pn532, uid, uid_length, &g_peak_info)) {
        HAL_UART_Transmit(&huart4, (uint8_t*)"[PN532 Task] *** 读取成功 ***\r\n", 32, 1000);
    } else {
        HAL_UART_Transmit(&huart4, (uint8_t*)"[PN532 Task] *** 读取失败 ***\r\n", 32, 1000);
    }
